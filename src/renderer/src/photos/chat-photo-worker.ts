...
const worker = self as unknown as { onmessage: ((event: MessageEvent<{ bytes: Uint8Array; maxEdge: number; quality: number }>) => void) | null; postMessage(value: unknown, transfer?: Transferable[]): void }
// MorsePhotoSendCropRenderer: the picture turned upright, its long side limited, and a JPEG without metadata.
worker.onmessage = event => {
  void (async () => {
    const { bytes: input, maxEdge, quality } = event.data
    ...
    try {
      ...
      catch { throw new Error('이 사진 파일을 열지 못했습니다. 다른 사진을 선택해 주세요.') }
      if (!bitmap.width || !bitmap.height || bitmap.width * bitmap.height > maxBackgroundPixels) throw new Error('지원하는 사진 크기를 확인해 주세요.')
      const ratio = Math.min(1, maxEdge / Math.max(bitmap.width, bitmap.height)), width = Math.max(1, Math.round(bitmap.width * ratio)), height = Math.max(1, Math.round(bitmap.height * ratio))
