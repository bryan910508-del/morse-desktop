...
async function bytes(response: Response, max: number, signal: AbortSignal, validate: () => void, progress?: (loaded: number) => void): Promise<Buffer> {
  ...
  if (response.status !== 200 || !response.body || (length !== null && (!Number.isSafeInteger(length) || length <= 0 || length > max))) {
    await response.body?.cancel(); throw new Error('Invalid story photo response')
  }
  ...
  try {
    while (true) {
      ...
      if (total > max) throw new Error('Story photo response too large')
      chunks.push(Buffer.from(part.value))
      if (Date.now() - lastProgress >= 500) { progress?.(total); lastProgress = Date.now() }
    }
    if (!total || (length !== null && total !== length)) throw new Error('Incomplete story photo response')
    return Buffer.concat(chunks, total)
  } finally { await reader.cancel().catch(() => {}); reader.releaseLock(); chunks.forEach(chunk => chunk.fill(0)) }
}
export async function downloadOwnStoryPhoto(auth: ReadCredentials, path: string, storyId: string, uid: string, signal: AbortSignal, validate: () => void, maxBytes: number, progress: (loaded: number, total: number) => void): Promise<Buffer> {
  if (!path.startsWith(`stories/user/${uid}/`)) throw new Error('Story photo scope mismatch')
  ...
  try {
    ...
    if (wire.error !== undefined || result.ok !== true || result.resourceURL !== resourceURL) throw new Error('Story photo authorization denied')
  ...
  const metadata = async () => {
    ...
    try {
      ...
      if (value.bucket !== storageBucket || value.name !== path || custom.storyId !== storyId || custom.ownerUid !== uid ||
          ...
          typeof value.md5Hash !== 'string' || !/^[A-Za-z0-9+/]{22}==$/.test(value.md5Hash)) throw new Error('Story photo metadata mismatch')
  ...
  try {
    ...
    if (JSON.stringify(before) !== JSON.stringify(after) || before.size !== downloaded.length ||
        createHash('md5').update(downloaded).digest('base64') !== before.hash) throw new Error('Story photo changed during download')
