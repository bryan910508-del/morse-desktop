...
async function bytes(response: Response, max: number, signal: AbortSignal, validate: () => void, progress?: (loaded: number) => void): Promise<Buffer> {
  ...
  if (response.status !== 200 || !response.body || (length !== null && (!Number.isSafeInteger(length) || length <= 0 || length > max))) {
    await response.body?.cancel(); throw new Error('Invalid channel media response')
  }
  ...
  try {
    while (true) {
      ...
      if (total > max) throw new Error('Channel media response too large')
      chunks.push(Buffer.from(part.value))
      if (Date.now() - lastProgress >= 500) { progress?.(total); lastProgress = Date.now() }
    }
    if (!total || (length !== null && total !== length)) throw new Error('Incomplete channel media response')
    return Buffer.concat(chunks, total)
  } finally { await reader.cancel().catch(() => {}); reader.releaseLock(); chunks.forEach(chunk => chunk.fill(0)) }
}
export async function downloadChannelPostMedia(auth: ReadCredentials, path: string, postId: string, signal: AbortSignal, validate: () => void, maxBytes: number, progress: (loaded: number, total: number) => void): Promise<Buffer> {
  ...
  try {
    ...
    if (wire.error !== undefined || result.ok !== true || result.resourceURL !== resourceURL) throw new Error('Channel media authorization denied')
  ...
  const metadata = async () => {
    ...
    try {
      ...
      if (value.bucket !== storageBucket || value.name !== path || custom.postId !== postId ||
          ...
          typeof value.md5Hash !== 'string' || !/^[A-Za-z0-9+/]{22}==$/.test(value.md5Hash)) throw new Error('Channel media metadata mismatch')
      return { size: Number(value.size), generation: value.generation, metageneration: value.metageneration, hash: value.md5Hash }
    } finally { payload.fill(0) }
  }
  ...
  try {
    ...
    if (JSON.stringify(before) !== JSON.stringify(after) || before.size !== downloaded.length ||
        createHash('md5').update(downloaded).digest('base64') !== before.hash) throw new Error('Channel media changed during download')
