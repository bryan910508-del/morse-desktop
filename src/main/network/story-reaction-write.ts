...
export async function writeStoryReaction(client: TransactionClient, auth: Metadata, uid: string, input: StoryReactionChangeRequest, signal: AbortSignal, validate: () => void): Promise<void> {
  ...
  try {
    ...
    const rows = await new Promise<Map<string, FirestoreDocument | null>>((done, reject) => {
      ...
      const cancel = (): void => finish(new Error('Reaction lookup cancelled'))
      ...
      call.on('data', (raw: WireObject) => {
        ...
        try {
          ...
          if (bytes > 4 * 1024 * 1024) throw new Error('Reaction lookup too large')
          ...
          if (typeof path !== 'string' || !paths.includes(path) || result.has(path)) throw new Error('Reaction lookup scope mismatch')
          result.set(path, doc)
        } catch (error) { finish(error) }
      })
      call.on('error', finish)
      call.on('end', () => finish(result.size === paths.length ? undefined : new Error('Incomplete reaction lookup')))
      signal.addEventListener('abort', cancel, { once: true }); if (signal.aborted) cancel()
    })
    ...
    signal.throwIfAborted(); validate()
    committing = true
    const result = await unary('commit', { database, transaction, writes: [{ update: { name: storyPath, fields }, updateMask: { fieldPaths: [ownField] }, currentDocument: { updateTime: story.updateTime } }] })
