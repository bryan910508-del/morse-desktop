...
async function metadata(response: Response): Promise<unknown> {
  if (!response.body) throw new Error(tr('미디어 완료 응답이 없습니다.'))
  const reader = response.body.getReader(), chunks: Uint8Array[] = []; let size = 0
  try { while (true) { const item = await reader.read(); if (item.done) break; size += item.value.byteLength; if (size > 65536) throw new Error(tr('미디어 완료 응답이 너무 큽니다.')); chunks.push(item.value) }; return JSON.parse(Buffer.concat(chunks).toString('utf8')) }
  finally { await reader.cancel().catch(() => {}); reader.releaseLock() }
}
export async function uploadStoryVideo(auth: ReadCredentials, photo: StoryVideoUploadSource, outer: AbortSignal, saveSession: (session: string) => Promise<void>, progress: (bytes: number) => void, validate: () => void): Promise<StoryVideoReceipt> {
  const { intent, part, transfer, bytes } = photo, path = storyVideoPath(intent.ownerId, intent.id, part)
  if (transfer.state !== 'pending' || transfer.receipt) throw new Error(tr('현재 미디어 업로드 기록을 확인해 주세요.'))
  ...
  let session = transfer.session ? storyVideoSession(transfer.session, intent.ownerId, intent.id, part) : null
  if (!session) {
    const response = await request(`https://firebasestorage.googleapis.com/v0/b/${storageBucket}/o?name=${encodeURIComponent(path)}`, { 'X-Goog-Upload-Protocol': 'resumable', 'X-Goog-Upload-Command': 'start', 'X-Goog-Upload-Header-Content-Length': String(bytes.byteLength), 'X-Goog-Upload-Header-Content-Type': storyVideoMime(part), 'Content-Type': 'application/json; charset=utf-8' }, JSON.stringify({ name: path, contentType: storyVideoMime(part), metadata: { ownerUid: intent.ownerId, storyId: intent.id } }))
    const raw = response.headers.get('X-Goog-Upload-URL'), active = response.headers.get('X-Goog-Upload-Status')
    await response.body?.cancel()
    if (response.status !== 200 || active !== 'active') throw new Error(tr('미디어 업로드 세션 시작 응답을 확인하지 못했습니다.'))
    session = storyVideoSession(raw, intent.ownerId, intent.id, part)
  ...
  if ([404, 410].includes(query.status)) throw new StoryVideoUploadBlocked('expired')
  ...
  if (!Number.isSafeInteger(offset) || offset > bytes.byteLength || (status === 'active' && offset !== bytes.byteLength && offset % 262144 !== 0)) throw new Error(tr('서버 업로드 위치가 준비 미디어과 다릅니다.'))
  if (status === 'final') throw new StoryVideoUploadBlocked('unknown') // Query is not a metadata acknowledgement; prepublication GET is forbidden by the current contract.
  progress(offset)
  while (true) {
    ...
    if (response.status !== 200 || response.headers.get('X-Goog-Upload-Status') !== (final ? 'final' : 'active')) { await response.body?.cancel(); throw new Error(tr('미디어 전송 응답을 확인하지 못했습니다. 자동 반복하지 않습니다.')) }
    if (final) { const receipt = storyVideoMetadata(await metadata(response), intent, part); validate(); signal.throwIfAborted(); progress(end); return receipt }
