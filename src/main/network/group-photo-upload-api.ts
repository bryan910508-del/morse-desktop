...
import { groupPhotoUploadPath, groupPhotoUploadSession, groupPhotoUploadURL, type GroupPhotoUploadRecord } from '../storage/group-photo-upload-table'
...
export async function uploadGroupPhoto(auth: ReadCredentials, uid: string, photo: GroupPhotoUploadRecord, signal: AbortSignal,
  saveSession: (session: string) => Promise<void>, progress: (value: number) => void, validate: () => void): Promise<string> {
  const path = groupPhotoUploadPath(photo.chatId, photo.id), objectURL = `https://firebasestorage.googleapis.com/v0/b/${storageBucket}/o/${encodeURIComponent(path)}`
  ...
  const metadata = async (): Promise<string | null> => {
    ...
    if (raw.bucket !== storageBucket || raw.name !== path || raw.contentType !== 'image/jpeg' || Number(raw.size) !== photo.bytes.byteLength ||
      raw.md5Hash !== photo.md5 || custom.ownerUid !== uid || custom.chatId !== photo.chatId || custom.morseUploadId !== photo.id || custom.morseSourceSHA256 !== photo.sha256 ||
    ...
    return groupPhotoUploadURL(`${objectURL}?alt=media&token=${encodeURIComponent(token)}`, photo.chatId, photo.id)
  }
  ...
  let session = photo.session ? groupPhotoUploadSession(photo.session, photo.chatId, photo.id) : ''
  if (!session) {
    const start = await request(`https://firebasestorage.googleapis.com/v0/b/${storageBucket}/o?name=${encodeURIComponent(path)}`, {
      ...
      body: JSON.stringify({ name: path, contentType: 'image/jpeg', metadata: { ownerUid: uid, chatId: photo.chatId, morseUploadId: photo.id, morseSourceSHA256: photo.sha256 } })
    })
    ...
    session = groupPhotoUploadSession(raw, photo.chatId, photo.id)
