...
async function bytes(response: Response, max: number, signal: AbortSignal, validate: () => void, progress?: (loaded: number) => void): Promise<Buffer> {
  ...
  if (response.status !== 200 || !response.body || (length !== null && (!Number.isSafeInteger(length) || length <= 0 || length > max))) {
    await response.body?.cancel(); throw new Error('Invalid story audio response')
  }
  ...
  try {
    while (true) {
      ...
      if (total > max) throw new Error('Story audio response too large')
      chunks.push(Buffer.from(part.value))
      if (Date.now() - lastProgress >= 500) { progress?.(total); lastProgress = Date.now() }
    }
    if (!total || (length !== null && total !== length)) throw new Error('Incomplete story audio response')
...
export async function downloadOwnStoryAudio(auth: ReadCredentials, path: string, storyId: string, uid: string, signal: AbortSignal, validate: () => void, maxBytes: number, progress: (loaded: number, total: number) => void): Promise<{ bytes: Buffer; mime: string }> {
  ...
  const metadata = async () => {
    ...
    try {
      ...
      if (value.bucket !== storageBucket || value.name !== path || custom.storyId !== storyId || custom.ownerUid !== uid ||
          typeof value.contentType !== 'string' || !['audio/mp4', 'audio/mpeg', 'audio/aac', 'audio/wav', 'audio/x-caf'].includes(value.contentType) ||
          ...
          typeof value.md5Hash !== 'string' || !/^[A-Za-z0-9+/]{22}==$/.test(value.md5Hash)) throw new Error('Story audio metadata mismatch')
  ...
  try {
    ...
    if (JSON.stringify(before) !== JSON.stringify(after) || before.size !== downloaded.length ||
        createHash('md5').update(downloaded).digest('base64') !== before.hash) throw new Error('Story audio changed during download')
    signal.throwIfAborted(); validate(); progress(downloaded.length, downloaded.length)
    return { bytes: downloaded, mime: before.contentType }
