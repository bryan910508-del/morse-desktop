...
import { profileUploadPath, profileUploadSession, profileUploadURL, type ProfileUploadRecord } from '../storage/profile-photo-upload-table'
...
export async function uploadProfilePhoto(auth: ReadCredentials, uid: string, photo: ProfileUploadRecord, signal: AbortSignal,
  saveSession: (session: string) => Promise<void>, progress: (value: number) => void, validate: () => void): Promise<string> {
  const path = profileUploadPath(uid, photo.id), objectURL = `https://firebasestorage.googleapis.com/v0/b/${storageBucket}/o/${encodeURIComponent(path)}`
  ...
  const metadata = async (): Promise<string | null> => {
    ...
    if (raw.bucket !== storageBucket || raw.name !== path || raw.contentType !== 'image/jpeg' || Number(raw.size) !== photo.bytes.byteLength ||
      raw.md5Hash !== photo.md5 || custom.ownerUid !== uid || custom.morseUploadId !== photo.id || custom.morseSourceSHA256 !== photo.sha256 ||
    ...
    return profileUploadURL(`${objectURL}?alt=media&token=${encodeURIComponent(token)}`, uid, photo.id)
  }
  ...
  let session = photo.session ? profileUploadSession(photo.session, uid, photo.id) : ''
  if (!session) {
    const start = await request(`https://firebasestorage.googleapis.com/v0/b/${storageBucket}/o?name=${encodeURIComponent(path)}`, {
      ...
      body: JSON.stringify({ name: path, contentType: 'image/jpeg', metadata: { ownerUid: uid, morseUploadId: photo.id, morseSourceSHA256: photo.sha256 } })
    })
    ...
    session = profileUploadSession(raw, uid, photo.id)
