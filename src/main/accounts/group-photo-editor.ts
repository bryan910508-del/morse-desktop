import type { GroupPhotoClear, GroupPhotoResult } from '../../shared/group-photo'
import { FirestoreReader, type ReadCredentials } from '../network/firestore-rpc'
import { GroupPhotoWriteFailure } from '../network/group-photo-write'
import type { FirestoreDocument } from '../network/firestore-values'
import { tr } from '../../shared/i18n'

export class GroupPhotoEditor {
  private closed = false
  private selection: string | null = null
  private active: { request: GroupPhotoClear; abort: AbortController; task: Promise<GroupPhotoResult> } | null = null
  private attempts = new Map<string, { request: GroupPhotoClear; task: Promise<GroupPhotoResult> }>()
  constructor(private readonly auth: ReadCredentials, private readonly source: (request: GroupPhotoClear, exact: boolean) => FirestoreDocument) {}
  ...
  save(request: GroupPhotoClear): Promise<GroupPhotoResult> {
  ...
  private async write(request: GroupPhotoClear, signal: AbortSignal): Promise<GroupPhotoResult> {
    let reader: FirestoreReader | null = null
    try {
      reader = new FirestoreReader(this.auth)
      await reader.clearGroupPhoto(request, signal, () => this.source(request, true))
      return { outcome: 'saved', message: tr('그룹 대표 사진 해제 응답을 확인했습니다. 최신 서버 정보에 반영되며 원본 파일은 유지됩니다.') }
    } catch (error) {
      return { outcome: error instanceof GroupPhotoWriteFailure && error.uncertain ? 'uncertain' : 'rejected',
        message: error instanceof GroupPhotoWriteFailure ? error.message : tr('그룹 사진 해제를 시작하지 못했습니다. 연결과 최신 그룹 정보를 확인해 주세요.') }
