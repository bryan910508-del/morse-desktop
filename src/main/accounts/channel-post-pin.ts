import type { ChannelPostPinEdit, ChannelPostPinResult } from '../../shared/channel-post-pin-edit'
import { FirestoreReader, type ReadCredentials } from '../network/firestore-rpc'
import { ChannelPostPinFailure } from '../network/channel-post-pin-write'
import type { ChannelPinWriteSource } from '../network/channel-post-pin-write'
import { tr } from '../../shared/i18n'

export class ChannelPostPinEditor {
  private closed = false
  private selection: string | null = null
  private active: { request: ChannelPostPinEdit; abort: AbortController; task: Promise<ChannelPostPinResult> } | null = null
  private attempts = new Map<string, { request: ChannelPostPinEdit; task: Promise<ChannelPostPinResult> }>()
  constructor(private readonly uid: string, private readonly auth: ReadCredentials, private readonly source: (request: ChannelPostPinEdit, exact: boolean) => ChannelPinWriteSource) {}
  ...
  save(request: ChannelPostPinEdit): Promise<ChannelPostPinResult> {
  ...
  private async write(request: ChannelPostPinEdit, signal: AbortSignal): Promise<ChannelPostPinResult> {
    let reader: FirestoreReader | null = null
    try {
      reader = new FirestoreReader(this.auth)
      await reader.saveChannelPostPin(this.uid, request, signal, () => this.source(request, true))
      return { outcome: 'saved', message: tr('채널 고정 대상과 변경 대상 게시물 표시를 함께 저장한 응답을 확인했습니다. 현재 값은 이후 다른 변경을 포함할 수 있습니다.') }
    } catch (error) {
      return { outcome: error instanceof ChannelPostPinFailure && error.uncertain ? 'uncertain' : 'rejected',
        message: error instanceof ChannelPostPinFailure ? error.message : tr('게시물 고정 변경을 시작하지 못했습니다. 현재 게시물과 접근 권한을 확인해 주세요.') }
