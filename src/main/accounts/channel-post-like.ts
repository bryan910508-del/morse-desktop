import type { ChannelPostLikeRequest, ChannelPostLikeResult } from '../../shared/channel-post-like'
import { FirestoreReader, type ReadCredentials } from '../network/firestore-rpc'
import { ChannelPostLikeFailure } from '../network/channel-post-like-write'
import type { FirestoreDocument } from '../network/firestore-values'
import { tr } from '../../shared/i18n'

export class ChannelPostLikeEditor {
  private closed = false
  private selection: string | null = null
  private active: { request: ChannelPostLikeRequest; abort: AbortController; task: Promise<ChannelPostLikeResult> } | null = null
  private attempts = new Map<string, { request: ChannelPostLikeRequest; task: Promise<ChannelPostLikeResult> }>()
  constructor(private readonly uid: string, private readonly auth: ReadCredentials, private readonly source: (request: ChannelPostLikeRequest, exact: boolean) => FirestoreDocument) {}
  ...
  save(request: ChannelPostLikeRequest): Promise<ChannelPostLikeResult> {
  ...
  private async write(request: ChannelPostLikeRequest, signal: AbortSignal): Promise<ChannelPostLikeResult> {
    let reader: FirestoreReader | null = null
    try {
      reader = new FirestoreReader(this.auth)
      await reader.setChannelPostLike(this.uid, request, signal, () => this.source(request, true))
      return { outcome: 'saved', message: tr('좋아요 변경 응답을 확인했습니다. 현재 목록은 이후 다른 변경을 포함할 수 있습니다.') }
    } catch (error) {
      return { outcome: error instanceof ChannelPostLikeFailure && error.uncertain ? 'uncertain' : 'rejected',
        message: error instanceof ChannelPostLikeFailure ? error.message : tr('좋아요 변경을 시작하지 못했습니다. 현재 게시물과 접근 권한을 확인해 주세요.') }
