import { contactAudienceStoryPhotoRequest, type ContactAudienceStoryPhotoRequest, type ContactAudienceStoryPhotoSnapshot } from '../../shared/contact-audience-story-photo'
...
export class ContactAudienceStoryPhoto {
  private closed = false
  private job: Promise<void> | null = null
  private selected: { request: ContactAudienceStoryPhotoRequest; ownerId: string; viewer: OwnStoryPhoto } | null = null
  constructor(private readonly uid: string, private readonly auth: ReadCredentials,
    private readonly prepare: (request: ContactAudienceStoryPhotoRequest) => { ownerId: string; story: OwnStoryDetail; path: string | null },
    private readonly contact: (request: ContactAudienceStoryPhotoRequest) => string, private readonly cache: StoryMediaCache, private readonly changed: () => void) {}
  private validate(request: ContactAudienceStoryPhotoRequest, ownerId: string): void {
    this.auth.signal.throwIfAborted()
    if (this.closed || this.selected?.request.selectionId !== request.selectionId || this.selected.ownerId !== ownerId || this.contact(request) !== ownerId) throw new Error(tr('선택한 연락처의 사진 읽기가 변경되었습니다.'))
  }
  get snapshot(): ContactAudienceStoryPhotoSnapshot | null {
  ...
  open(input: ContactAudienceStoryPhotoRequest): Promise<void> {
    const request = contactAudienceStoryPhotoRequest(input)
    if (this.closed || this.job) throw new Error(tr('진행 중인 청중 사진 읽기를 마친 뒤 다시 선택해 주세요.'))
    const prepared = this.prepare(request)
    if (prepared.ownerId === this.uid || prepared.story.mediaType !== (request.presentation === 'video-poster' ? 'video' : 'image') || (request.presentation === 'video-poster' && !prepared.story.hasThumbnail) || prepared.story.privacy !== request.privacy) throw new Error(tr('현재 연락처의 청중 사진 스토리를 확인해 주세요.'))
    this.pause()
    ...
    const viewer = new OwnStoryPhoto(prepared.ownerId, this.auth, current => {
      ...
      if (current.selectionId !== raw.selectionId || current.requestId !== raw.requestId || current.storyId !== raw.storyId || current.version !== raw.version || current.presentation !== raw.presentation) throw new Error(tr('청중 사진 선택이 변경되었습니다.'))
      return prepared.story
    }, () => { if (!this.closed) this.changed() }, { prefix: '__contact-audience-story-photo', inspect: doc => { if (storyHiddenFrom(doc).hiddenFrom.includes(this.uid)) throw new Error(tr('청중 스토리 숨김 설정이 변경되었습니다.')) }, cache: this.cache, address: () => this.prepare(request).path })
  ...
  preload(input: ContactAudienceStoryPhotoRequest): Promise<void> {
    const request = contactAudienceStoryPhotoRequest(input)
    if (this.closed) throw new Error(tr('현재 연락처의 청중 사진 스토리를 확인해 주세요.'))
    const prepared = this.prepare(request)
    if (prepared.ownerId === this.uid || prepared.story.mediaType !== (request.presentation === 'video-poster' ? 'video' : 'image') || (request.presentation === 'video-poster' && !prepared.story.hasThumbnail) || prepared.story.privacy !== request.privacy) throw new Error(tr('현재 연락처의 청중 사진 스토리를 확인해 주세요.'))
    ...
    const viewer = new OwnStoryPhoto(prepared.ownerId, this.auth, current => {
      this.auth.signal.throwIfAborted()
      if (this.closed || this.contact(request) !== prepared.ownerId) throw new Error(tr('현재 연락처의 청중 사진 스토리를 확인해 주세요.'))
      if (current.selectionId !== raw.selectionId || current.requestId !== raw.requestId || current.storyId !== raw.storyId || current.version !== raw.version || current.presentation !== raw.presentation) throw new Error(tr('청중 사진 선택이 변경되었습니다.'))
      return this.prepare(request).story
    }, () => {}, { prefix: '__contact-audience-story-photo', inspect: doc => { if (storyHiddenFrom(doc).hiddenFrom.includes(this.uid)) throw new Error(tr('청중 스토리 숨김 설정이 변경되었습니다.')) }, cache: this.cache, address: () => this.prepare(request).path })
