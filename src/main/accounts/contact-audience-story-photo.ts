...
export class ContactAudienceStoryPhoto {
  ...
  constructor(private readonly uid: string, private readonly auth: ReadCredentials,
    private readonly prepare: (request: ContactAudienceStoryPhotoRequest) => { ownerId: string; story: OwnStoryDetail },
    private readonly contact: (request: ContactAudienceStoryPhotoRequest) => string, private readonly changed: () => void) {}
  ...
  open(input: ContactAudienceStoryPhotoRequest): Promise<void> {
    ...
    }, () => { if (!this.closed) this.changed() }, { prefix: '__contact-audience-story-photo', inspect: doc => { if (storyHiddenFrom(doc).hiddenFrom.includes(this.uid)) throw new Error(tr('청중 스토리 숨김 설정이 변경되었습니다.')) } })
