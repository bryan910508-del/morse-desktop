import type { ChannelTagsEdit, ChannelTagsResult } from '../../shared/channel-tags'
import { FirestoreReader, type ReadCredentials } from '../network/firestore-rpc'
import { ChannelTagsWriteFailure } from '../network/channel-tags-write'
import type { FirestoreDocument } from '../network/firestore-values'
import { tr } from '../../shared/i18n'

export class ChannelTagsEditor {
  private closed = false
  private selection: string | null = null
  private active: { request: ChannelTagsEdit; abort: AbortController; task: Promise<ChannelTagsResult> } | null = null
  private attempts = new Map<string, { request: ChannelTagsEdit; task: Promise<ChannelTagsResult> }>()
  constructor(private readonly auth: ReadCredentials, private readonly source: (request: ChannelTagsEdit, exact: boolean) => FirestoreDocument) {}
  ...
  save(request: ChannelTagsEdit): Promise<ChannelTagsResult> {
  ...
  private async write(request: ChannelTagsEdit, signal: AbortSignal): Promise<ChannelTagsResult> {
    let reader: FirestoreReader | null = null
    try {
      reader = new FirestoreReader(this.auth)
      await reader.setChannelTags(request, signal, () => this.source(request, true))
      return { outcome: 'saved', message: tr('채널 태그 저장 응답을 확인했습니다. 최신 서버 목록에 반영됩니다.') }
    } catch (error) {
      return { outcome: error instanceof ChannelTagsWriteFailure && error.uncertain ? 'uncertain' : 'rejected',
        message: error instanceof ChannelTagsWriteFailure ? error.message : tr('채널 태그 저장을 시작하지 못했습니다. 연결과 최신 채널 정보를 확인해 주세요.') }
